 ...
 * and that the theme's assets are actually loadable from the FusionAuth login page. The hosted login HTML must
 * reference the Latte Java logo served by the app's static server (proving the custom theme is in effect rather than
 * FusionAuth's stock default theme), and the app must serve {@code /static} assets with a Cross-Origin-Resource-Policy
 * that permits the cross-origin FusionAuth login page to embed them — without weakening that policy anywhere else.
 *
  ...
  public void staticAssetsAllowCrossOriginEmbedding() throws Exception {
    try (HttpClient client = HttpClient.newBuilder().followRedirects(HttpClient.Redirect.NEVER).build()) {
      HttpResponse<Void> logo = client.send(
          HttpRequest.newBuilder(URI.create("http://localhost:" + Main.PORT + "/static/images/logo.svg")).GET().build(),
          HttpResponse.BodyHandlers.discarding());
      Assert.assertEquals(logo.statusCode(), 200,
          "Expected the static logo to be served, got [" + logo.statusCode() + "]");
      ...
      HttpResponse<Void> appPage = client.send(
          HttpRequest.newBuilder(URI.create("http://localhost:" + Main.PORT + "/")).GET().build(),
          HttpResponse.BodyHandlers.discarding());
